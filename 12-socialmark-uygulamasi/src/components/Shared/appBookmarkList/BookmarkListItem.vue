  ...
  <div class="bg-white flex flex-col gap-x-3 rounded-md shadow-sm">
    <div class="p-3">
      ...
      <div class="flex items-center justify-center mt-2 gap-x-1">
        ...
        <button @click="bookmarkItem" :class="{ 'bookmark-item-active': alreadyBookmarked }" class="bookmark-btn group">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="fill-current group-hover:text-white"
...
export default {
  ...
  methods: {
    ...
    bookmarkItem() {
      this.$appAxios({
        ...
        let bookmarks = [...this.$userStore._userBookmarks];
        if (this.alreadyBookmarked) {
          bookmarks = bookmarks.filter((b) => b.id !== this.bookmarkedItem.id);
        } else {
          bookmarks = [...bookmarks, res.data];
        }
        this.$userStore.setBookmarks(bookmarks);
      });
    },
    // likeItem() {
    //   let likes = [...this.$userStore._userLikes];

    //   if (!this.alreadyLiked) {
    //     likes = [...likes, this.item.id];
    //   } else {
    //     likes = likes.filter((i) => i !== this.item.id);
    //   }
    //   this.$appAxios.patch(`/users/${this.$userStore._getCurrentUser.id}`, { likes }).then((res) => {
    //     this.$userStore.setLikes(likes);
    //   });
    // },
    // bookmarkItem() {
    //   let bookmarks = [...this.$userStore._userBookmarks];
    ...
    //   } else {
    //     bookmarks = bookmarks.filter((i) => i !== this.item.id);
    //   }
    //   this.$appAxios.patch(`/users/${this.$userStore._getCurrentUser.id}`, { bookmarks }).then((res) => {
    //     this.$userStore.setBookmarks(bookmarks);
    //   });
    // },
  },
  computed: {
    ...
    alreadyLiked() {
      // return this.$userStore._userLikes?.indexOf(this.item.id) > -1;
      return Boolean(this.likedItem);
    },
    alreadyBookmarked() {
      // return this.$userStore._userBookmarks?.indexOf(this.item.id) > -1;
      return Boolean(this.bookmarkedItem);
    },
    likedItem() {
      return this.$userStore._userLikes?.find((i) => i.bookmarkId === this.item.id);
    },
    bookmarkedItem() {
      return this.$userStore._userBookmarks?.find((i) => i.bookmarkId === this.item.id);
    },
  },
};
</script>
